...
class SelectorDIC(ModelSelector):
    ...
    def select(self):
        ...
        for n_components in range(self.min_n_components, self.max_n_components + 1):
            try:
                model = self.base_model(n_components)
                logL_current_word = model.score(self.X, self.lengths)
                logL_rest_words = []
                for word in self.hwords:
                    if word != self.this_word:
                        X, lengths = self.hwords[word]
                        logL_rest_words.append(model.score(X, lengths))
                logL_rest_words_average = np.average(logL_rest_words)
                DIC = logL_current_word - logL_rest_words_average
                if DIC > best_score:
                    best_score = DIC
                    best_model = model
            except:
...
class SelectorCV(ModelSelector):
    ...
    def select(self):
        ...
        for n_components in range(self.min_n_components, self.max_n_components + 1):
            try:
                scores = []
                splits = KFold().split(self.sequences)
                ...
                if scores_average > best_score:
                    best_score = scores_average
